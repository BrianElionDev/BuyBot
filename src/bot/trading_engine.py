...
class TradingEngine:
    ...
    async def calculate_5_percent_take_profit(self, coin_symbol: str, position_type: str, entry_price: float) -> Optional[float]:
        ...
        try:
            ...
            trading_pair = self.binance_exchange.get_futures_trading_pair(coin_symbol)
            symbol_filters = await self.binance_exchange.get_futures_symbol_filters(trading_pair)
            precision = symbol_filters.get('quantityPrecision', 0) if symbol_filters else 0
            rounded_take_profit_price = round(take_profit_price, precision)

            return rounded_take_profit_price

        except Exception as e:
            logger.error(f"Error calculating 5% take profit for {coin_symbol}: {e}")
            return None
    ...
    async def ensure_take_profit_for_position(self, coin_symbol: str, position_type: str, position_size: float, entry_price: float, external_tp: Optional[float] = None) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if external_tp is not None and external_tp > 0:
                ...
                tp_price = await self.calculate_5_percent_take_profit(coin_symbol, position_type, entry_price)
                if not tp_price:
    ...
    async def _cancel_existing_take_profit_orders(self, trading_pair: str) -> bool:
        ...
        try:
            ...
            cancelled_count = 0
            for order in open_orders:
                if order.get('symbol') == trading_pair and order.get('type') == 'TAKE_PROFIT_MARKET':
                    order_id = order.get('orderId')
                    if order_id:
                        cancel_result = await self.binance_exchange.cancel_futures_order(trading_pair, order_id)
                        if cancel_result:
