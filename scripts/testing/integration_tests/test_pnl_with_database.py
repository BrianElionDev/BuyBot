#!/usr/bin/env python3
"""
P&L Testing with Database and Order Lifecycle Matching

This script matches database trades with Binance income history using
the exact order lifecycle (created_at to modified_at) for accurate P&L tracking.

Usage:
    python scripts/test_pnl_with_database.py --symbol ETH --days 7
"""
...
class OrderLifecyclePnLTester:
    ...
    def analyze_matched_trades(self, matched_trades: List[Dict]) -> Dict:
        ...
        for matched in matched_trades:
            ...
            net_pnl = matched.get('total_net_pnl', binance_pnl + matched['total_commission'] + matched['total_funding_fee'])
            ...
            result = {
                ...
                'net_pnl': net_pnl,  # NET P&L including fees
